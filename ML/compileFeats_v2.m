...
%path = 'C:\Users\danie\OneDrive\Desktop\Pathology\Images';
...
mapSurv = containers.Map('KeyType', 'double', 'ValueType', 'double');
dbstop if error
for x = 1: length(numbers(:, idCol)) %we want the the second column which corresponds to the slide IDs
    if (isnan(numbers(x,idCol))) 
        ...
        for y = 1: length(splitInd)
            ...
            mapSurv(str2double(id)) = numbers(x, lengthSurvivalCol);
        end
       ...
       mapSurv(double(numbers(x, idCol))) = numbers(x, lengthSurvivalCol);   
    end
end
