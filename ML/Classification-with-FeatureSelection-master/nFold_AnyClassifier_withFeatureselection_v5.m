...
%             train=tra{1}; test=tes{1};
    ...
    for i=1:n
        ...
        training_set = training_set(:, ~all(isnan(training_set))); %edited by daniel. do i need to adjust the coresponding label?
        ...
        if strcmp(para.featureranking,'ttest') | strcmp(para.featureranking,'wilcoxon')
            ...
            if strcmp(para.featureranking,'ttest')
                %% change made by daniel shao to remove any columns with all NaN
