...
function [stats, feature_scores]= nFold_AnyClassifier_withFeatureselection_v5_daniel(data_set,data_labels,feature_list,para,shuffle,n,nIter,Subsets)
...
%         training_set = training_set(:, ~any(isnan(training_set))); %edited by daniel. do i need to adjust the coresponding label?
%         testing_set = testing_set(:, ~any(isnan(training_set))); %could make it ~all if i wanted to be safer, but that's given me some problems
        ...
        if strcmp(para.featureranking,'mrmr')
            ...
            [idx_TTest] = mrmr_mid_d_daniel(dataw_discrete(:,setAll), training_labels, para.num_top_feature); %changed by daniel shao
        end
        ...
        if strcmp(para.featureranking,'ttest') | strcmp(para.featureranking,'wilcoxon')
            ...
            if strcmp(para.featureranking,'ttest')
                [TTidx,confidence] = prunefeatures_new_daniel(training_set, training_labels, 'ttestp');
            ...
            if strcmp(para.featureranking,'wilcoxon')
                [TTidx,confidence] = prunefeatures_new_daniel(training_set, training_labels, 'wilcoxon');
